...
/**
 * Description of BonDeTransfert
 *
 * @author MIRADO
 */
defined('BASEPATH') OR exit('No direct script access allowed');

class BonDeTransfert extends MyFPDF {
...
$this->myfpdf = new BonDeTransfert();
$titre = 'Bon de Transfert du ' . $transfert->getDateTransfertFormat();
...
//if (!empty($porte)) {
    ...
    $test1['text'][0] = 'Transfert ' . $transfert->getTransfert();
...
$this->myfpdf->SetFont('Times', '', 12);
    ...
    $this->myfpdf->Cell(0, 10, utf8_decode($transfert->getCommentaire()), 1, 1, 'C');
...
$data = [];
for ($i = 0; $i < count($sousTransferts); $i++) {
    $array = [];
    $array[0] = utf8_decode($sousTransferts[$i]->getDesignationMateriel());
    $array[1] = $sousTransferts[$i]->getQuantite();
...
$this->myfpdf->SetX(55);
$this->myfpdf->Cell(50, 7, "Porte Source: ". $transfert->getNumeroPorteSource() , 0, 'C');
$this->myfpdf->Cell(50, 7, "Porte Destinataire: ".$transfert->getNumeroPorteDest(), 0, 'C');
...
$this->myfpdf->Output('D', 'Bon_de_transfert.pdf');